        ...
        post(`/payctcfcg/create-result`).then(response => {
            if (response.res == responseStatus.success) {
                if (response.data.status == 2) {
                    ...
                    post(`/loanctcf/contract-bank-info`, {loan_type}).then(response2 => {
                        response2 = {
                            "res": 1,
                            "data": {
                        ...
                        if (response2.res == responseStatus.success) {
                            dispatch({type: 'receiveOnlineBankInfo', detail: response2.data})
